...
public class ColorArcBar extends View {
	...
	private int[] startColor = new int[] { Color.parseColor("#aefc65"),
			Color.parseColor("#62f1f4"), Color.parseColor("#6cf2e0"),
			Color.parseColor("#7ff5bc"), Color.parseColor("#97f98f"),
			Color.parseColor("#aefc65") };

	private int[] endColor = new int[] { Color.parseColor("#aefc65") };
	...
	public ColorArcBar(Context context) {
		super(context, null);
		initView();
	}

	public ColorArcBar(Context context, AttributeSet attrs) {
		super(context, attrs, 0);
		initCofig(context, attrs);
		initView();
	}

	public ColorArcBar(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	private void initCofig(Context context, AttributeSet attrs) {
		 ...
		 * 适配不同分辨率的设备
		 */
		diameter = (int) (DensityUtils.getDensityWidth(context) * 0.4);
	...
	private void initView() {
		...
		contentPaint.setTextSize(dipToPx(40));
		contentPaint.setColor(Color.parseColor("#a7c6c4"));
		contentPaint.setTextAlign(Paint.Align.CENTER);

		// 显示标题文字
		titlePaint = new Paint();
		titlePaint.setTextSize(dipToPx(20));
		titlePaint.setColor(Color.parseColor("#a7c6c4"));
		...
		sweepGradient = new SweepGradient(centerX, centerY, startColor, null);
	...
	protected void onDraw(Canvas canvas) {
		...
		contentPaint.setColor(Color.parseColor("#ffffff"));
		if (!TextUtils.isEmpty(contentString))
			canvas.drawText(contentString, centerX, centerY + contentSize + 30,
					contentPaint);
		canvas.drawText("综合得分", centerX, centerY + contentSize + 200,
				titlePaint);
