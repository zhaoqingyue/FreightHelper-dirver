...
public class BottomBarTab extends FrameLayout {
    ...
    private void init(Context context, int icon, CharSequence title) {
       ...
       // int size = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 27, getResources().getDisplayMetrics());
        ...
        mTvTitle.setTextColor(ContextCompat.getColor(context, R.color.bar_text_color));
        mTvTitle.setLayoutParams(paramsTv);
        lLContainer.addView(mTvTitle);

        addView(lLContainer);

        int min = (int)(iconsize/4); // dip2px(context, 20);
       // int padding = dip2px(context, 5);
        mTvUnreadCount = new TextView(context);
        mTvUnreadCount.setBackgroundResource(R.drawable.img_readpoint);
...
//        tvUnReadParams.leftMargin = (int)(iconsize/1.5);//dip2px(context, 17);
//      tvUnReadParams.bottomMargin = (int)(iconsize/1.5);//dip2px(context, 14);
       tvUnReadParams.leftMargin = (int)(iconsize/2.5);//dip2px(context, 17);
       tvUnReadParams.bottomMargin = (int)(iconsize/1.6);//dip2px(context, 14);
    ...
    public void setSelected(boolean selected) {
        ...
        if (selected) {
          //  mIcon.setColorFilter(ContextCompat.getColor(mContext, R.color.colorPrimary));
            mTvTitle.setTextColor(ContextCompat.getColor(mContext, R.color.bar_text_color_select));
        } else {
          //  mIcon.setColorFilter(ContextCompat.getColor(mContext, R.color.tab_unselect));
            mTvTitle.setTextColor(ContextCompat.getColor(mContext, R.color.bar_text_color));
        }
    }
     ...
     * 获取当前未读数量
     */
    public boolean getUnreadCountVisible() {

        return (mTvUnreadCount.getVisibility() == View.VISIBLE);
    }
