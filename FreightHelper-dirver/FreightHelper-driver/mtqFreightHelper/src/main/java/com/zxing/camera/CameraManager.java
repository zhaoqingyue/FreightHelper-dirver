...
public final class CameraManager {
	...
	private CameraManager(Context context) {
		...
		// in the Donut SDK.
		// useOneShotPreviewCallback = NaviAppUtil.parseInt(Build.VERSION.SDK) >
	 ...
	 * @return boolean
	 * @author hejie
	 * @date 2015年7月1日 上午10:57:10
	 */
	public static boolean checkCamera(Context context) {
		if (context.getPackageManager().hasSystemFeature(
				PackageManager.FEATURE_CAMERA)) {
			CldLog.p("Camera is Exist");
		} else {
			CldLog.p("Camera is not Exist");
			return false;
		}
		...
		try {
			c = Camera.open();
			CldLog.p("Camera can open");
			c.release();
		} catch (Exception e) {
			CldLog.p("Camera can not open");
			return false;
		}
		...
		// } catch (Exception e) {
		// isReady = false;
		// CldLog.p("Camera preview can not open");
		// }
		// if(isReady){
		// c.stopPreview();
		// CldLog.p("Camera preview can open");
		// }
		return isReady;
	}
	...
	public void openDriver(SurfaceHolder holder) throws IOException {
		if (camera == null) {
			...
			// 默认会添加，但魅族M351等有些没有添加，需要添加处理，否则会异常退出 wenap 20140614
	...
	public Rect getFramingRect() {
		...
		if (framingRect == null) {
			...
			int width = (int) (screenResolution.x * 610.0 / 1080.0);
			int height = (int) (screenResolution.y * 615.0 / 1920.0);
			if (height > screenResolution.x) {
				height = width;
			} else
				height = width = (width > height) ? width : height;
			int leftOffset = (int) (234.0 / 1080.0 * screenResolution.x);
			int topOffset = (int) (610.0 / 1920.0 * screenResolution.y);
